...
export default function UbicacionPickerModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="space-y-2">
              {zonas.map(zona => (
                <button
                  ...
                  className={`w-full text-left px-5 py-4 rounded-2xl border font-bold transition-all ${
              ...
              {zonas.length === 0 && (
                <div className="text-slate-500 text-sm text-center py-4 bg-slate-900/50 rounded-xl border border-slate-800/50 border-dashed">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 min-h-[300px]">
              {!selectedRackId && (
                ...
                <div className="grid grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 pb-2">
                  {espaciosDisponibles.map(espacio => (
                    <button
                      ...
                      className="group flex flex-col items-center justify-center p-4 bg-slate-900 border border-slate-800 rounded-xl hover:border-green-500 hover:bg-green-500/10 hover:text-green-400 transition-all text-slate-300 active:scale-95"
                    >
                      <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest group-hover:text-green-600/50 transition-colors mb-1">Cuna</span>
        ...
        {/* Footer actions */}
        <div className="px-8 py-5 border-t border-slate-800/60 bg-slate-900/50 flex justify-between items-center">
          <button
            type="button"
            onClick={() => {
              // Unassign (put afloat)
            ...
            className="text-slate-400 hover:text-white font-medium text-sm transition-colors py-2 px-4 rounded-xl hover:bg-slate-800"
          >
