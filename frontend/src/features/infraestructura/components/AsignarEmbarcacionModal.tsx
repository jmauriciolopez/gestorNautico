      ...
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800/60 bg-slate-900/50">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Seleccione la embarcación que ocupará el espacio <strong className="text-white bg-slate-800 px-2 py-0.5 rounded px-1">{codigoEspacio}</strong>
            </p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
          {embarcacionesLibres.length === 0 ? (
            <div className="text-center py-10 bg-slate-950/50 rounded-2xl border border-dashed border-slate-800">
              ...
              <p className="text-slate-400 font-medium">No hay embarcaciones "a flote" o sin ubicación registrada.</p>
              <p className="text-slate-500 text-sm mt-1">Desea agregar una nueva embarcación desde su módulo correspondiente o liberar una de otro rack.</p>
            </div>
          ) : (
                ...
                <button
                  ...
                  className={`text-left px-5 py-4 rounded-2xl border transition-all ${
                    selectedId === emb.id 
                      ...
                      : 'bg-slate-950 border-slate-800 text-slate-300 hover:border-blue-500/50 hover:bg-slate-900'
                  }`}
                >
                  <p className="font-bold text-lg">{emb.nombre}</p>
                  <p className={`text-sm ${selectedId === emb.id ? 'text-blue-100' : 'text-slate-500'}`}>
        ...
        <div className="px-6 py-4 border-t border-slate-800/60 bg-slate-900/50 flex justify-end gap-3">
          ...
          <button
            ...
            className="px-6 py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-bold rounded-xl transition-all shadow-lg active:scale-95 flex items-center gap-2"
          >
